...
public class EventMethods {
	...
	private static WebElement getElement(By by,int timesout) {
		WebElement e;
		try {
			WebDriverWait w = new WebDriverWait(Data.get(), timesout);
	...
	public static void switchToFrame(String str,int timeout) {
		WebDriverWait w =new WebDriverWait(Data.get(),timeout );
		w.pollingEvery(Duration.ofMillis(200));
	...
	public static void list_value(By by,String str,int timeout) {
		...
		List<WebElement> l=Data.get().findElements(by);
		for(WebElement e:l) {
	...
	public static void switch_Window() {
		...
		Set<String> windows=Data.get().getWindowHandles();
		for(String s: windows) {
			if(!s.equalsIgnoreCase(Data.get().getWindowHandle())) {
